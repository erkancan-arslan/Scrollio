...
import { simulateBotBattleScore } from './botLogic';
import {
    BilVeFethetState,
    BattleState,
    TurnResult,
    ...
    switch (action.type) {
        ...
        case 'CLAIM_REGION': {
            ...
            if (nextIndex >= REGIONS.length) {
                // All 12 regions claimed — start battle phase
        ...
        case 'DEFENSE_TIME_UP': {
            ...
            return {
                ...
                lastResult: result,
                phase: 'result',
                botTurnIndex: state.botTurnIndex + 1,
            };
        }

        case 'ACKNOWLEDGE_RESULT': {
            if (state.phase !== 'result') return state;
            // If we were defending, botTurnIndex was already incremented in DEFENSE_TIME_UP
            // If we were attacking (player attacked bot), start bots fresh from index 0
        ...
        case 'BOT_NEUTRAL_CLAIM': {
            ...
            // Increment botTurnIndex and stay in bot_turn — effect re-runs for next bot
            return {
                ...state,
                ownership: newOwnership,
                botTurnIndex: state.botTurnIndex + 1,
